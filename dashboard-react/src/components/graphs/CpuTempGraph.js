...
function CpuTempGraph({ data, setActiveData }) {
  const filtered = data.filter(d => d.temp_cpu !== undefined);
  ...
  const option = {
    tooltip: {
      ...
      formatter: function (params) {
        ...
        return `
          ...
          Temp. Battery: ${d.temp_bat != null ? (d.temp_bat / 100).toFixed(1) : '-'} °C<br/>
    ...
    series: [
      {
        data: filtered.map(d => ({ ...d, value: d.temp_cpu })),
        type: 'line',
        showSymbol: true,
        smooth: true,

...
function CurrentGraph({ data, setActiveData }) {
  const filtered = data.filter(d => d.inst_curr !== undefined);
  ...
  const option = {
    ...
    series: [
      {
        data: filtered.map(d => ({ ...d, value: d.inst_curr })),
        type: 'line',
        showSymbol: false,
        smooth: true,
        lineStyle: { color: '#1b3fa5' }
      }
    ]
  };
